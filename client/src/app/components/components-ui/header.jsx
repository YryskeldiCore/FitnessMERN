	...
	function getPhrase() {
		if (currentUser) {
			if (currentUser.isAdmin) {
				return `Приветствуем админа ${currentUser.name}!`
			}
			return `Приветствуем тебя ${currentUser.name}!`
		}
		return "Приветствуем тебя дорогой пользователь!"
	}
	...
	return (
		<header className="header">
			...
			<div className="header__container _container header__container_second">
					...
					{!noController && (
						<React.Fragment>
							...
							<TextField
								...
								name="search"
								label={<img src="/img/header/search.svg" alt="Search icon" />}
								placeholder="search"
								onChange={onChange}
				...
				{widthWindow >= 940 && (
					<div className="header__greetings">{getPhrase()}</div>
				)}
